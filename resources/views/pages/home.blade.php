...
<section class="featured-services-section py-5">
    <div class="container">
        <div class="text-center mb-5">
            <div class="badge bg-pink-soft text-pink fw-semibold px-3 py-2 rounded-pill mb-3 d-inline-block">
                <i class="bi bi-scissors me-1"></i> Signature Services
            </div>
            <h2 class="display-5 fw-bold mb-3">
                Our Most Loved<br>
                <span class="text-gradient">Braiding Styles</span>
            </h2>
            <p class="lead text-muted max-w-2xl mx-auto">
                A glimpse of the styles our clients request most. Explore the full collection to find your perfect fit.
            </p>
        </div>
        ...
        <div class="row g-4 justify-content-center">
            ...
            <div class="col-md-6 col-lg-3">
                <a href="{{ route('services.show', $service->slug) }}" class="home-service-card d-block h-100 text-decoration-none">
                    <div class="service-icon-wrap bg-pink-soft rounded-circle d-flex align-items-center justify-content-center mb-3">
                        <i class="bi {{ $service->icon_class ?? 'bi-scissors' }} fs-3 text-pink"></i>
                    </div>
                    <h4 class="h5 fw-bold mb-2">{{ $service->name }}</h4>
                    <p class="text-muted small mb-3">{{ $service->description }}</p>
                    @if($service->starting_price)
                    <div class="fw-bold text-pink">From R{{ number_format($service->starting_price, 0) }}</div>
                    @endif
                </a>
            </div>
            @endforeach
        ...
        <div class="text-center mt-5">
            <a href="{{ route('services.index') }}" class="btn btn-outline-pink btn-lg px-4 py-3">
...
<!-- FEATURED PORTFOLIO -->
@if($featuredStyles->count())
<section class="featured-portfolio-section py-5 bg-pink-soft">
    <div class="container">
        <div class="text-center mb-5">
            <div class="badge bg-white text-pink fw-semibold px-3 py-2 rounded-pill mb-3 d-inline-block">
                <i class="bi bi-images me-1"></i> Gallery Highlights
            </div>
            <h2 class="display-5 fw-bold mb-3">
                Recent<br>
            ...
            <p class="lead text-muted max-w-2xl mx-auto">
                Every braid tells a story. Take a look at some of our favorite recent transformations.
            </p>
        </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6 col-lg-4">
                <a href="{{ route('portfolio.show', $style->slug) }}" class="home-portfolio-card d-block rounded-4 overflow-hidden shadow-sm text-decoration-none">
                    <div class="home-portfolio-image-wrap">
                        <img src="{{ $style->image_url }}" alt="{{ $style->title }}" class="img-fluid w-100" style="height: 260px; object-fit: cover;">
                    </div>
                    <div class="p-3 bg-white">
                        <h5 class="mb-0 fw-semibold text-dark">{{ $style->title }}</h5>
                        <span class="small text-muted">{{ ucwords(str_replace('-', ' ', $style->category)) }}</span>
                    </div>
                </a>
            </div>
            @endforeach
        ...
        <div class="text-center mt-5">
            <a href="{{ route('portfolio') }}" class="btn btn-pink btn-lg px-4 py-3">
                <i class="bi bi-images me-2"></i> View Full Portfolio
            </a>
        </div>
    </div>
</section>
...
<section class="testimonials-section py-5">
    <div class="container">
        <div class="text-center mb-5">
            <div class="badge bg-pink-soft text-pink fw-semibold px-3 py-2 rounded-pill mb-3 d-inline-block">
                <i class="bi bi-chat-quote me-1"></i> Client Love
            </div>
            <h2 class="display-5 fw-bold mb-3">
                What Our Clients<br>
                <span class="text-gradient">Are Saying</span>
            </h2>
        </div>
        ...
        <div class="row g-4 justify-content-center">
            ...
            <div class="col-md-6 col-lg-4">
                <div class="testimonial-card bg-white rounded-4 p-4 h-100 shadow-sm">
                    <i class="bi bi-quote fs-1 text-pink opacity-25 d-block mb-2"></i>
                    <p class="fst-italic mb-4">"{{ $testimonial->client_feedback }}"</p>
                    <div class="d-flex align-items-center">
                        <div class="avatar-sm rounded-circle bg-pink-soft text-pink d-flex align-items-center justify-content-center fw-bold me-2">
                            {{ $testimonial->client_initials ?? '?' }}
                        </div>
                        <div>
                            <div class="small fw-semibold">{{ $testimonial->client_initials ?? 'Happy Client' }}</div>
                            <div class="x-small text-muted">{{ $testimonial->title }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="position-relative" style="z-index: 2;">
                ...
                <div class="d-flex flex-column flex-sm-row gap-3 justify-content-center">
                    <a href="{{ route('contact') }}" class="btn btn-light btn-lg px-4 py-3 text-pink fw-bold">
                        <i class="bi bi-calendar-check fs-5 me-2"></i> Book Appointment
                    </a>
                    <a href="{{ route('portfolio') }}" class="btn btn-outline-light btn-lg px-4 py-3">
                        <i class="bi bi-images fs-5 me-2"></i> View Portfolio
                    </a>
                </div>
            </div>
        </div>
...
<style>
    ...
    .home-service-card:hover {
        ...
        border-color: rgba(212, 86, 135, 0.25);
    }
    .service-icon-wrap {
        width: 72px;
        height: 72px;
    ...
    .home-portfolio-image-wrap img {
        transition: transform 0.5s ease;
    }
    .home-portfolio-card:hover .home-portfolio-image-wrap img {
    ...
    .testimonial-card:hover {
        ...
        box-shadow: 0 15px 30px rgba(212, 86, 135, 0.12);
    }
    .avatar-sm {
        width: 36px;
        height: 36px;
    ...
    /* CTA */
    .home-cta-section .cta-card {
        box-shadow: 0 20px 50px rgba(212, 86, 135, 0.25);
    }

    @media (max-width: 768px) {
        .home-service-card { padding: 1.5rem 1rem; }
    }
</style>
@endsection